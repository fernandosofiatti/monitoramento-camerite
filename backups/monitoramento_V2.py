...
COLUNAS_PAINEL            = 4
DATA_PARSE_VERSION = "2026-05-06-iso-us-v2"
...
def preencher_cidade_estado_por_clientes(df: pd.DataFrame, clientes_prefeitura: dict) -> pd.DataFrame:
    ...
    cols = {normalizar_coluna(c): c for c in df.columns}
    city_col = next((cols[k] for k in ("cidade", "municipio", "city") if k in cols), "cidade")
    state_col = next((cols[k] for k in ("estado", "uf", "state") if k in cols), None)
...
def parse_ultima_atualizacao(coluna: pd.Series) -> pd.Series:
    """
    Converte a data da Camerite preservando o formato do CSV.
    O GOV exporta datas como ISO/americano (ex.: 2026-04-01 ou 04/01/2026 = 1 de abril).
    """
    ...
    if mask.any():
        parsed.loc[mask] = valores.loc[mask].apply(
            lambda valor: pd.to_datetime(valor, errors="coerce", dayfirst=False)
        )
...
def montar_mapa_cidades(df: pd.DataFrame) -> tuple[go.Figure | None, str]:
    ...
    cols = {normalizar_coluna(c): c for c in df.columns}
    city_col = next((cols[k] for k in ("cidade", "municipio", "city") if k in cols), None)
    lat_col = next((cols[k] for k in ("latitude", "lat") if k in cols), None)
    lon_col = next((cols[k] for k in ("longitude", "long", "lon") if k in cols), None)

    state_col = next((cols[k] for k in ("estado", "uf", "state") if k in cols), None)
    if city_col is None:
        if state_col is not None:
            return montar_mapa_estados(df, state_col)
        return None, "Inclua uma coluna de cidade (Cidade / Municipio / City) ou estado (Estado / UF / State) no arquivo para mapear geograficamente."

    df_map = df.copy()
    df_map[city_col] = df_map[city_col].astype(str).str.strip()
    df_map = df_map[df_map[city_col] != ""].copy()
    if df_map.empty:
        return None, "Não há cidades válidas para exibir no mapa."
    ...
    if "lat" not in df_map.columns or "lon" not in df_map.columns or df_map["lat"].isna().all() or df_map["lon"].isna().all():
        coords = {}
        if state_col:
            unique_pairs = df_map[[city_col, state_col]].drop_duplicates()
            for _, row in unique_pairs.iterrows():
                city = str(row[city_col]).strip()
                state = str(row[state_col]).strip()
                if not city:
                    continue
                coord = geocode_cidade(city, state if state else None)
                if coord:
                    coords[(city, state)] = coord
            df_map["coord"] = df_map.apply(
                lambda r: coords.get((str(r[city_col]).strip(), str(r[state_col]).strip())),
                axis=1
            )
        else:
            cidades = sorted(df_map[city_col].dropna().unique())
            for cidade in cidades:
                coord = geocode_cidade(cidade)
                if coord:
                    coords[cidade] = coord
            df_map["coord"] = df_map[city_col].map(coords)

        if df_map["coord"].isna().all():
            return None, "Não foi possível geocodificar as cidades. Adicione colunas Latitude/Longitude ou verifique o nome das cidades."

        df_map = df_map[df_map["coord"].notna()].copy()
        if df_map.empty:
            return None, "Nenhuma cidade foi geocodificada com sucesso."
        df_map[["lat", "lon"]] = pd.DataFrame(df_map["coord"].tolist(), index=df_map.index)

    df_group = df_map.groupby(df_map[city_col].astype(str).str.strip(), as_index=False).agg(
        total=(COL_STATUS, "size"),
        offline=(COL_STATUS, lambda x: (x.astype(str).str.strip().str.upper() == "OFFLINE").sum()),
        lat=("lat", "first"),
        lon=("lon", "first"),
    )
    df_group["Pct"] = df_group.apply(lambda row: row["offline"] / row["total"] * 100 if row["total"] else 0, axis=1)
    df_group = df_group.sort_values("Pct", ascending=False)

    if df_group.empty:
        return None, "Não há dados agrupados por cidade para o mapa."
    ...
    fig.add_trace(go.Scattergeo(
        lon=df_group["lon"], lat=df_group["lat"],
        text=df_group[city_col],
        marker=dict(
            size=(5 + df_group["Pct"] * 0.6).clip(5, 40),
            color=df_group["Pct"],
        ...
        hovertemplate="<b>%{text}</b><br>%{customdata[0]:.1f}% offline<br>%{customdata[1]} offline de %{customdata[2]}<extra></extra>",
        customdata=df_group[["Pct", "offline", "total"]].values,
        name="Cidades",
    ))

    capitals = BRAZIL_STATE_CAPITAIS
    fig.add_trace(go.Scattergeo(
        lon=[data["lon"] for data in capitals.values()],
        lat=[data["lat"] for data in capitals.values()],
        text=[data["city"] for data in capitals.values()],
        mode="markers+text",
    ...
    return fig, f"Mapa geográfico exibindo {len(df_group)} cidades com dados de lat/lon."


def montar_mapa_estados(df: pd.DataFrame, state_col: str) -> tuple[go.Figure | None, str]:
    df_state = df.copy()
    df_state[state_col] = df_state[state_col].astype(str).str.strip()
    if df_state[state_col].eq("").all():
        return None, "Não há estados válidos para exibir no mapa."

    df_state["state_name"] = df_state[state_col].apply(obter_nome_estado)
    df_state = df_state[df_state["state_name"].notna()].copy()
    if df_state.empty:
        return None, "Não foi possível reconhecer os estados. Use colunas Estado / UF / State com valores válidos."

    df_group = df_state.groupby("state_name", as_index=False).agg(
        total=(COL_STATUS, "size"),
        offline=(COL_STATUS, lambda x: (x.astype(str).str.strip().str.upper() == "OFFLINE").sum()),
    )
    df_group["Pct"] = df_group.apply(lambda row: row["offline"] / row["total"] * 100 if row["total"] else 0, axis=1)
    ...
    geojson = carregar_geojson_estados()
    if geojson is None:
        return None, "Não foi possível carregar o mapa de estados do Brasil. Verifique a conexão ou o arquivo cache."
...
def calcular_saude_dataframe(df: pd.DataFrame | None, clientes_map: dict, origem: str = "Arquivo local") -> dict:
    if df is None or df.empty:
        return {
            ...
            "sem_data": 0, "ultima_data": "N/D", "arquivo_atualizado": "N/D",
            "colunas_faltando": [],
        }
    ...
    if clientes_map and COL_WL in df_meta.columns:
        ids_validos = set(clientes_map.keys())
        linhas_processadas = df_meta[COL_WL].astype(str).str.strip().isin(ids_validos).sum()

    datas_invalidas = 0
    sem_data = 0
    ultima_data = "N/D"
    if COL_ULT_ATU in df_meta.columns:
        valores = df_meta[COL_ULT_ATU].astype("string").str.strip()
        parsed = parse_ultima_atualizacao(df_meta[COL_ULT_ATU])
...
def montar_fila_acao(df_clientes: pd.DataFrame) -> pd.DataFrame:
    if df_clientes.empty:
        return pd.DataFrame()
    cols = ["Cliente", "Franqueado", "Offline", "Total", "% Offline", "Maior Tempo", "Acima 24h",
            "Dias Críticos", "Delta Offline", "_score"]
    df = df_clientes[df_clientes["Offline"] > 0].copy()
    if df.empty:
        return df
    return df.sort_values(["_score", "% Offline", "Offline"], ascending=False)[cols]
...
def render_card(col, wl_id, v, tendencia, delta_off):
    ...
    id_html  = f'<div style="font-size:9px;color:#6b8496">ID: {wl_id}</div>'
...
def main():
    ...
    df_fila_acao = montar_fila_acao(df_clientes_ops)
    ...
    with tabs[0]:
        ...
        st.markdown("**Atuar agora**")
        if df_fila_acao.empty:
            st.success("Nenhum cliente com câmera offline no momento.")
        else:
            df_acao = df_fila_acao.head(10).copy()
            df_acao = df_acao.rename(columns={"_score": "Prioridade"})
            df_acao["% Offline"] = df_acao["% Offline"].apply(lambda x: f"{x:.1f}%")
            df_acao["Delta Offline"] = df_acao["Delta Offline"].apply(
                lambda x: "N/D" if pd.isna(x) else f"{int(x):+d}"
            )
            render_dataframe(df_acao, height=min(420, (len(df_acao)+1)*35+3))

            buf_acao = io.BytesIO()
            df_fila_acao.drop(columns=["_score"], errors="ignore").to_excel(buf_acao, index=False, engine="openpyxl")
            st.download_button(
                "⬇ Exportar fila de ação",
                data=buf_acao.getvalue(),
                file_name=f"fila_acao_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        ...
        with col_gauge:
            ...
            fig_g = go.Figure(go.Indicator(
                ...
                gauge=dict(
                    ...
                    steps=[
                        dict(range=[0,10],   color="#dff8f3"),
                        dict(range=[10,15],  color="#fef3c7"),
                        dict(range=[15,100], color="#fee2e2"),
                    ],
    ...
    with tabs[1]:
        ...
        col_search, col_franq, col_filtro, col_min, col_ordem = st.columns([2,2,2,1,1])
        with col_search:
            busca = st.text_input("Buscar", placeholder="Buscar cliente ou franqueado…")
        with col_franq:
            filtro_franq = st.selectbox("Franqueado", franqueados)
        with col_filtro:
            filtro = st.selectbox("Status", STATUS_CLIENTE)
        with col_min:
            min_cameras = st.selectbox("Min. câmeras", [0, 10, 50, 100, 200])
        with col_ordem:
            ordem = st.selectbox("Ordenar", ["Prioridade", "% offline", "Qtd offline", "Nome"])
        ...
        if ordem == "Prioridade":
            ids_ord = df_clientes_ops.sort_values("_score", ascending=False)["ID"].tolist()
        elif ordem == "% offline":
            ids_ord = df_clientes_ops.sort_values("% Offline", ascending=False)["ID"].tolist()
        elif ordem == "Qtd offline":
            ids_ord = df_clientes_ops.sort_values("Offline", ascending=False)["ID"].tolist()
        else:
            ids_ord = df_clientes_ops.sort_values("Cliente", ascending=True)["ID"].tolist()
