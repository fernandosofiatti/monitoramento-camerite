...
@st.cache_data(ttl=60)
def carregar_dados(pasta: str, parse_version: str = DATA_PARSE_VERSION) -> tuple[dict, str]:
    """
    ...
    with tabs[0]:
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:1.5rem">
            <div style="background:#ffffff;border:1px solid #dbe8f2;border-radius:8px;padding:14px 16px">
                <div style="font-size:10px;color:#6b8496;font-weight:700;text-transform:uppercase;letter-spacing:.7px">CSV processado</div>
                <div style="font-size:24px;color:#0088cc;font-family:'DM Mono',monospace;font-weight:700">{saude.get("linhas_processadas",0)}</div>
                <div style="font-size:11px;color:#6b8496">{saude.get("linhas_csv",0)} linhas lidas</div>
            </div>
            <div style="background:#ffffff;border:1px solid #dbe8f2;border-radius:8px;padding:14px 16px">
                <div style="font-size:10px;color:#6b8496;font-weight:700;text-transform:uppercase;letter-spacing:.7px">Último sinal no CSV</div>
            ...
            <div style="background:#ffffff;border:1px solid #dbe8f2;border-radius:8px;padding:14px 16px">
                <div style="font-size:10px;color:#6b8496;font-weight:700;text-transform:uppercase;letter-spacing:.7px">Saúde do dado</div>
                <div style="font-size:24px;color:{'#ef4444' if saude.get('datas_invalidas',0) else '#14b8a6'};font-family:'DM Mono',monospace;font-weight:700">{saude.get("datas_invalidas",0)}</div>
                <div style="font-size:11px;color:#6b8496">{saude.get("sem_data",0)} sem data · {saude.get("clientes_xlsx",0)} clientes XLSX</div>
            </div>
            <div style="background:#ffffff;border:1px solid #dbe8f2;border-radius:8px;padding:14px 16px">
                <div style="font-size:10px;color:#6b8496;font-weight:700;text-transform:uppercase;letter-spacing:.7px">Variação vs anterior</div>
                <div style="font-size:24px;color:{'#ef4444' if delta_global > 0 else ('#14b8a6' if delta_global < 0 else '#0088cc')};font-family:'DM Mono',monospace;font-weight:700">{delta_global:+.0f}</div>
                <div style="font-size:11px;color:#6b8496">{clientes_melhoraram} melhoraram · {clientes_pioraram} pioraram</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
