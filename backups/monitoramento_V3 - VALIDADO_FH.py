...
def main():
    ...
    # Comparar os dois últimos snapshots manuais.
    ...
    if len(snapshot_ids) == 2:
        ...
        new_map = df_new.set_index("wl_id")[['offline','pct_offline']].to_dict(orient='index')
        old_map = df_old.set_index("wl_id")[['offline','pct_offline']].to_dict(orient='index')
        tendencias = {}
        delta_offs = {}
        for wl in dados:
    ...
    with tabs[0]:
        ...
        with col_gauge:
            st.markdown("**% Total de Câmeras Offline GOV**")
            ...
            fig_g = go.Figure(go.Indicator(
                ...
                gauge=dict(
                    ...
                    borderwidth=0,
                    steps=[
                        dict(range=[0,10],   color="#a7f3d0"),
                        dict(range=[10,15],  color="#fde68a"),
                        dict(range=[15,100], color="#fecaca"),
                    ],
        ...
        with col_pie:
            st.markdown("**Clientes por faixa de saúde**")
    ...
    with tabs[2]:
        ...
        if not rows_tempo:
            ...
            k1.markdown(f'''<div style="background:#ecfdf5;border:1px solid #99f6e4;border-radius:8px;padding:16px;text-align:center">
                <div style="font-size:10px;color:#6b8496;font-weight:600;text-transform:uppercase;letter-spacing:.7px">Menos de 1 dia</div>
                <div style="font-size:32px;font-weight:700;color:#14b8a6;font-family:DM Mono,monospace">{menos_1d}</div>
            ...
            k2.markdown(f'''<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:16px;text-align:center">
                <div style="font-size:10px;color:#6b8496;font-weight:600;text-transform:uppercase;letter-spacing:.7px">Entre 1 e 3 dias</div>
                <div style="font-size:32px;font-weight:700;color:#f59e0b;font-family:DM Mono,monospace">{entre_1_3d}</div>
                <div style="font-size:11px;color:#6b8496">câmeras em atenção</div></div>''', unsafe_allow_html=True)
            k3.markdown(f'''<div style="background:#fff1f2;border:1px solid #fecdd3;border-radius:8px;padding:16px;text-align:center">
                <div style="font-size:10px;color:#6b8496;font-weight:600;text-transform:uppercase;letter-spacing:.7px">3 a 7 dias</div>
                <div style="font-size:32px;font-weight:700;color:#ef4444;font-family:DM Mono,monospace">{entre_3_7d}</div>
            ...
            k4.markdown(f'''<div style="background:#ffffff;border:1px solid #dbe8f2;border-radius:8px;padding:16px;text-align:center">
                <div style="font-size:10px;color:#6b8496;font-weight:600;text-transform:uppercase;letter-spacing:.7px">Acima de 7 dias</div>
                <div style="font-size:32px;font-weight:700;color:#6b8496;font-family:DM Mono,monospace">{acima_7d}</div>
                <div style="font-size:11px;color:#6b8496">câmeras mais antigas</div></div>''', unsafe_allow_html=True)
            ...
            if df_exib.empty:
                ...
                if not df_graf.empty:
                    ...
                    cores_t = ["#dc2626" if h>=168 else ("#d97706" if h>=72 else ("#f59e0b" if h>=24 else "#059669")) for h in df_graf["_horas"]]
                    ...
                    fig_t.add_vline(x=24,  line_dash="dot", line_color="#f59e0b", line_width=1,
                                    annotation_text="1d", annotation_font=dict(color="#f59e0b",size=10))
                    fig_t.add_vline(x=72,  line_dash="dot", line_color="#dc2626", line_width=1,
                                    annotation_text="3d", annotation_font=dict(color="#dc2626",size=10))
                    fig_t.add_vline(x=168, line_dash="dot", line_color="#7c2d12", line_width=1,
                                    annotation_text="7d", annotation_font=dict(color="#7c2d12",size=10))
                    fig_t.update_layout(
