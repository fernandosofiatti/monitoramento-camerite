    ...
    with col:
        ...
        if count > 0:
            if st.button("🔎 Ver detalhes do cliente", key=f"btn_detalhe_cliente_{str(wl_id)}"):
                ...
                st.rerun()  # força rerun do app inteiro (não só do fragment da lista)
        else:
            st.button("✓ Operacional", key=f"btn_operacional_cliente_{str(wl_id)}", disabled=True)


@st.fragment
def render_lista_clientes_filtrada(dados: dict, df_clientes_ops: pd.DataFrame,
                                    tendencias: dict, delta_offs: dict) -> None:
    """Filtros (busca/franqueado/mín. câmeras/status) + grade de cards dos clientes.

    Isolado em st.fragment: aplicar um filtro (inclusive a busca) rerroda só este
    bloco, sem reprocessar o app inteiro (comparativo de snapshots, outras abas etc).
    """
...
def render_config_cidades_clientes() -> None:
    ...
    if st.button("💾 Salvar cidades/clientes", type="primary", key="cfg_clientes_salvar"):
        ...
        if ok:
            ...
            st.cache_data.clear()
            st.success("Cidades/clientes salvos.")
            st.rerun()
        else:
...
def render_aba_configuracao() -> None:
    ...
    sub_params, sub_cidades = st.tabs(["🎛️ Parâmetros", "🏙️ Cidades / Clientes"])

    with sub_cidades:
        render_config_cidades_clientes()

    with sub_params:
        _render_config_parametros()


def _render_config_parametros() -> None:
    ...
    with tabs["Clientes"]:
        ...
        with clientes_subtabs[0]:
            ...
            if "detalhe" not in st.session_state:
                ...
                render_lista_clientes_filtrada(dados, df_clientes_ops, tendencias, delta_offs)
